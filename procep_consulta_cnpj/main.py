...
def find_cnpj():
    ...
    data = pd.read_excel(path_cnpj, header=None, names=['CNPJ', 'RAZÃO SOCIAL', 'MATRIZ || FILIAL', 'TELEFONE', 'EMAIL', 'SITUAÇÃO CADASTRAL', 'DATA DE ABERTURA', 'ULTIMA ATUALIZAÇÃO', 'ENDEREÇO', 'CIDADE | UF', 'CEP'], converters={'CNPJ': '{:0>14}'.format})
    ...
    for cnpj_lista in tqdm(cnpjs):
        ...
        i += 1
        print(df)
    df.to_excel(f"C:\Matheus\www\\automacao-rgm-telecom\procep_consulta_cnpj\consulta.xlsx", index=False, header=True)
